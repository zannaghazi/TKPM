...
function MyTablePagination(props) {
	...
	useEffect(() => {
		console.log("list", props.listAccount);
		let amount = 0;
		if (props.listAccount.length % config.pagination.limit == 0) {
			amount = props.listAccount.length / config.pagination.limit;
		}
		else {
			amount = props.listAccount.length / config.pagination.limit + 1;
		}
