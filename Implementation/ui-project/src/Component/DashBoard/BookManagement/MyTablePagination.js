...
function MyTablePagination(props) {
	...
	useEffect(() => {
		console.log("list", props.listBook);
		let amount = 0;
		if (props.listBook.length % config.pagination.limit == 0) {
			amount = props.listBook.length / config.pagination.limit;
		}
		else {
			amount = props.listBook.length / config.pagination.limit + 1;
		}
