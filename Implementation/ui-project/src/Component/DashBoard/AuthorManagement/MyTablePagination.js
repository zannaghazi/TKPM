...
function MyTablePagination(props) {
	...
	useEffect(() => {
		...
		let amount = 0;
		if (props.listAuthor.length % config.pagination.limit == 0) {
			amount = props.listAuthor.length / config.pagination.limit;
		}
		else {
			amount = props.listAuthor.length / config.pagination.limit + 1;
		}
		console.log("ammount", amount, props.listAuthor.length);
		let temp = [];
		if (props.active == 1) {
